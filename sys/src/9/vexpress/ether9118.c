 ...
 *
 * igepv2 u-boot uses pin 64 on gpio 3 as an output pin to reset the 9118.
 */
#include "u.h"
#include "../port/lib.h"
#include "mem.h"
...
smcreset(Ctlr* ctlr)
{
	...
	maclo = macrd(regs,Macaddrl);
	machi = macrd(regs,Macaddrh);
	ctlr->ra[0]=maclo & 0xff;
	ctlr->ra[1]=(maclo>>8) & 0xff;
	ctlr->ra[2]=(maclo>>16) & 0xff;
	ctlr->ra[3]=(maclo>>24) & 0xff;
	ctlr->ra[4]=(machi) & 0xff;
	ctlr->ra[5]=(machi>>8) & 0xff;
...
//		memmove(edev->ea, ctlr->ra, Eaddrlen);
	...
	//regs->irqcfg = 1<<24 | Irqpushpull;  /* deas for 10µs (linux) */
	regs->irqcfg = 1<<24 | Irqen | Irqpushpull | Irqpol;  /* deas for 10µs (linux) */
	coherence();		/* back-to-back write/read delay per §6.2.1 */
...
smcpci(void)
{
	...
	vether=vmap(PHYSETHER,BY2PG);
	ctlr = malloc(sizeof(Ctlr));
	ctlr->id = Vid9118<<16 | 0x0424;	/* smsc 9118 */
	ctlr->port = (int)vether;
	ctlr->nic = (int *)vether;
	ctlr->regs = (Regs *)vether;
